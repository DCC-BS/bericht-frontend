import { AddComplaintItemCommand, RemoveComplaintItemCommand } from "~/models/commands";
...
export function useComplaintItemService(itemid: string) {
	...
	async function addItem(item: ComplaintItemInput) {
		...
		await executeCommand(new AddComplaintItemCommand(complaintItem, currentComplaint.value?.id));
	}
	...
	function removeItem(item: IComplaintItem) {
		...
		var command = new RemoveComplaintItemCommand(item, currentComplaint.value?.id);
	...
	onCommand<RemoveComplaintItemCommand>("RemoveComplaintItemCommand", async (command: RemoveComplaintItemCommand) => {
		if (!currentComplaint.value) {
			logger.error("Complaint not found");
			return;
		}
	...
	onCommand<AddComplaintItemCommand>("AddComplaintItemCommand", async (command: AddComplaintItemCommand) => {
		if (!currentComplaint.value) {
			logger.error("Complaint not found");
			return;
		}
